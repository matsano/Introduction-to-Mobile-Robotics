...
class ImageToWorld:
    ...
    def compute_dim(self):
        ...
        print("original dim world : ({}, {})".format(self._img_src.shape[1], self._img_src.shape[0]))
        print("self.x_max = {}, self.y_max = {}".format(self.x_max, self.y_max))
        print("auto_resized = {}".format(self._auto_resized))

        best_factor = self.height_world / self.y_max
        if not self._auto_resized:
            best_factor = 1.0
        best_width_world = int(round(best_factor * self.x_max))
        print("Best dim world : ({}, {}) with factor {}".format(best_width_world, self.height_world, best_factor))

        self.factor = self.height_world / self._img_src.shape[0]
        if not self._auto_resized:
            self.factor = 1.0
            self.height_world = self._img_src.shape[0]
        self.width_world = int(round(self.factor * self._img_src.shape[1]))
        print("Used dim world : ({}, {}) with factor {}".format(self.width_world, self.height_world, self.factor))
    ...
    def write_lines_and_boxes(self):
        ...
        f.write("\"\"\"\n")
        f.write("This file was generated by the tool 'image_to_world.py' in the directory tools.\n")
        f.write(
        ...
        if len(self.boxes) != 0:
            for i, box in enumerate(self.boxes):
                ...
                xw = x0 - self.width_world / 2
                yw = -y0 + self.height_world / 2
        ...
        if len(self.lines) != 0:
            for i, line in enumerate(self.lines):
                ...
                xw0 = x0 - self.width_world / 2
                yw0 = -y0 + self.height_world / 2

                xw1 = x1 - self.width_world / 2
                yw1 = -y1 + self.height_world / 2
